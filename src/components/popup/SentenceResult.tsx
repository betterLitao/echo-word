  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        {data.alternatives?.length ? <StatusPill icon={<Stack size={14} weight="duotone" />} label={`${data.alternatives.length + 1} 个引擎`} /> : null}
      </div>
      ...
      {data.alternatives?.length ? (
        <div className="space-y-3 rounded-[1.45rem] border border-white/10 bg-black/20 p-4">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500">对照结果</p>
          {data.alternatives.map((item) => (
            <div key={`${item.provider}-${item.translated_text}`} className="rounded-[1.1rem] border border-white/8 bg-white/[0.04] p-3">
              <div className="flex flex-wrap gap-2">
              ...
              {item.notice ? <p className="mt-2 text-xs leading-6 text-slate-400">{item.notice}</p> : null}
            </div>
          ))}
        </div>
      ) : null}
