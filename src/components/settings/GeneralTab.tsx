...
export function GeneralTab() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)]">
      <SectionCard title="通用设置" description="主题、代理和本地 HTTP API 都属于跨页面共享的系统配置，放在这里统一维护更直观。">
        <div className="grid gap-5 md:grid-cols-[1fr_1fr]">
          ...
          <Field label="数据目录" description="当前仍以默认应用目录为主，后续会接入迁移与切换流程。">
            <input className={fieldControlClassName} readOnly value={settings.data_dir} />
          </Field>
          <Field label="HTTP API 端口" description="本地只监听 127.0.0.1，方便 Alfred / Raycast / 脚本调用。">
      ...
      <SectionCard title="当前状态" description="这些开关都和后台能力联动：剪贴板监听会驱动 watcher，主题会同步主窗口与弹窗。">
        <div className="space-y-4">
          ...
          <div className="grid gap-3">
            <Button variant="secondary" onClick={() => void updateSetting('clipboard_listen', !settings.clipboard_listen)}>{settings.clipboard_listen ? '关闭剪贴板监听' : '开启剪贴板监听'}</Button>
            <Button variant="secondary" onClick={() => void updateSetting('auto_update', !settings.auto_update)}>{settings.auto_update ? '关闭自动更新' : '开启自动更新'}</Button>
            <Button variant="secondary" onClick={() => void updateSetting('privacy_mode', !settings.privacy_mode)}>{settings.privacy_mode ? '关闭隐私模式' : '开启隐私模式'}</Button>
          </div>
        </div>
      </SectionCard>
    </div>
